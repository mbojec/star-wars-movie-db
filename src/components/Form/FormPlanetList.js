        ...
        <div className={`col-12 form__info-section`}>
          {this.props.queryPlanets.length > 0 && this.state.searchHasFocus && (
            <ul className={'form__planet-list'}>
              {this.props.queryPlanets.map(singlePlanet => (
                <li onClick={() => this.savePlanet(singlePlanet)} key={singlePlanet.name} className={'form__planet-list__item'}>
                  {singlePlanet.name}
                </li>
              ))}
            </ul>
